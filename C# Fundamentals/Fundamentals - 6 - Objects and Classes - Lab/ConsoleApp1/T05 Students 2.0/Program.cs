    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (studentInformation[0] != "end")
            {
                ...
                //an overly complicated way to see if the name already exists in the list.
                string input = ($"{studentInformation[0]}  {studentInformation[1]}");
                if (toCheck.Contains(input))
                {
                    int toBeRemoved = toCheck.IndexOf(input);
                    toCheck.RemoveAt(toBeRemoved);
                    students.RemoveAt(toBeRemoved);
                }
